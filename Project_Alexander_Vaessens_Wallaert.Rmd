...
abstract: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque eu nibh at eros volutpat elementum a et metus. Morbi elementum molestie magna vitae hendrerit. In bibendum ante lacus, in posuere sem facilisis id. Proin tempus accumsan orci ac rutrum. Donec consectetur eleifend nibh id placerat. Curabitur pharetra dui enim, eu ultrices odio finibus non. Aenean tincidunt tellus congue fermentum tincidunt. Fusce a dolor varius, viverra lorem vitae, hendrerit metus. Phasellus tristique neque tortor. Sed sodales elit a dui commodo, et venenatis neque varius. Sed vitae mattis mi. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Nunc lobortis arcu sit amet leo elementum eleifend. Cras congue mattis tincidunt. Maecenas vitae sapien quis ligula laoreet eleifend."
---
...
tibble(scores = Z, reject = factor(RejectionStatus$Reject_Status, labels = c("Accepted", 
  ...
  labs(fill = "Rejection status") +
  theme(legend.position = "bottom") +
  geom_linerange(inherit.aes = F, aes(x=scores), ymin = 0, ymax=0.01 )
...
Nevertheless, two-sided Welch t-tests were performed on the uncentered data to determine whether the two groups can be differentiated based on the gene expression level for every gene. The choice for the Welch t-test is motivated by the presence of unequal variances and **differences in sample size** between the two groups, even though not all genes were normally distributed. We included a small random subset 6 QQ plots in the appendix so the reader, if she/he wishes, can have a rough idea of the divergence from normality.
...
hist(z_scores,breaks=120,prob=T,ylim=c(0,0.2), ylab=NULL, axes=FALSE, main='lfdr and z-scores')
par(new=TRUE)
...
```{r}
z.locfdr <- locfdr(z_scores, nulltype = 0, plot = 2)
