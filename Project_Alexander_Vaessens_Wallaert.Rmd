...
df <- tibble(patient = RejectionStatus$Patient_ID, 
             reject = ifelse(RejectionStatus$Reject_Status == 1, "Rejected", "Not Rejected"), 
             as.data.frame(t(X_GSE21374))) %>%
  pivot_longer(cols = c(-reject,-patient) , names_to = "gene", values_to = "expression")

# mean
df %>%
  group_by(reject, gene) %>%
  summarise(meanexpr = mean(expression)) -> mean_expression
  
ggplot(mean_expression, aes(meanexpr, fill = reject)) +
  geom_density(alpha = 0.4) +
  theme_classic() +
  labs(fill = "Status") -> plot_mean
