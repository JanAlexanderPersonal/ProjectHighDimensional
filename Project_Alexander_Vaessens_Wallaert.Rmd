...
abstract: "This research examines whether some genes are responsible for a patient's likelihood of rejecting a kidney after transplantation, for the  Gene Expression Omninibus (GEO) dataset. This dataset consists of gene expression levels of 54675 genes from 282 patients. The variability of the gene expression is ... From the 54675 genes, 18080 genes are identified as having a differential expression between the group of rejected and the group of accepted kidneys. The list of these genes can be found in ...(insert appendix) Kidney rejection can be predicted sufficiently from the gene expressions with 17 genes. The most inmportant genes in predicting rejection are in... (insert appendix). The prediction model (insert which model is best) perfoms the best and (say something about the performance)"
---
...
To summarise the results we constructed a histogram of both the regular and adjusted p-values or q-values. 
...
Both histograms reveal non-uniform distributions. More importantly, they have many small values, indicating that for many genes the null hypothesis was rejected. Based on the q-values, there were `r rejections` rejected null hypotheses. As such we conclude that the mean gene expression differs between the accepted and rejected kidney groups for those `r rejections` genes. As the FDR is controlled at 10%, it is expected to have around `r round(rejections*.1,0)` false discoveries.
...
From the figure \ref{lfdr} can be concluded that a small lfdr can be obtained for small and large z-scores (a lfdr smaller than 0.2 for z-scores smaller than -2 and larger than 2). For these z-scores, it is more likely that when rejecting the null hypotheses, a true discovery is made.
...
In the figure above, one can see that for $\gamma$ equal to   `r m.cv$lambda.min`  , the area under the curve ( _AUC_ ) is maximal (`r round(performance(pred_m, "auc")@y.values[[1]], digits = 3)`) for the train dataset based on a 10-fold cross-validation over the train dataset. 

The ROC curve, estimated with the cross-validation dataset, is shown below:

```{r performance lasso}
lasso_selection <- unique(summary(coef(m))[-1, 1])
...
Compared to the interesting genes found with the Sparse LDA method: `r intersect(lasso_selection, best_116)`
...
When performing the same analysis on this reduced dataset as was performed on the complete dataset, we see that the gene expressions become a bit more separable.

```{r descriptives for lasso, echo=FALSE, fig.cap="Descriptive statistics across grenes and between groups. Note that the data were centered before calculating the statistics."}
df <- tibble(patient = RejectionStatus$Patient_ID, 
             reject = ifelse(RejectionStatus$Reject_Status == 1, "Rejected", "Accepted"), 
             as.data.frame(GeneExpression_C)[, lasso_selection]) %>%
  pivot_longer(cols = c(-reject,-patient) , names_to = "gene", values_to = "expression")
...
Likewise as for the Lasso regression, for $\gamma$ equal to `r m.cv$lambda.min`, the optimal _AUC_ (`r round(performance(pred_m, "auc")@y.values[[1]], digits = 3)`) is obtained. 
...
colnames(tab) <- c('accept predicted', 'reject predicted')
rownames(tab) <- c('accept observed', 'reject observed')
knitr::kable(tab, digits = 0, align = 'l')
```
This confustionmatrix clearly shows a low sensitivity of `r tab[4] / (tab[3] + tab[4])`, but a high specificity of `r tab[1] / (tab[1] + tab[2])`. 
In short, this predictor can be trusted relatively well to predict a person will accept the kidney. 
If the test is positive however and it predicts the person will reject the kidney, there is a significant probability this is a false positive.
...
From the 54675 genes in the dataset, 18080 genes are differentially expressed between the two kidney groups, based on multi-scale Welch t-test at an FDR of 10%.
...
Locally linear embedding described by Roweis and Saul (2000) was performed. From figures \ref{lle} can be seen that no distinction between the accepted and rejected kidneys can be made.
...
ISOMAP presented by Tenenbaum, Silva and Langford in 2000 is performed. The parameter k is varied manually so that the maps are optimal. From figure \ref{ISOMAP} can be seen that with ISOMAP, it is also not possible to make a distinction between the group of accepted and rejected kidneys.
...
Sammon mapping presented by Sammon (1969). The result is in figures \ref{sammon}: no distinction can be made between the two groups.
...
Diffusion mapping was presented by Nadler et al (2006) and LAfon and Lee (2006). From figure \ref{diffusion}, no distinction between the two groups can be made with diffusion maps.
...
t-stochastic neighbor embedding is presented by Van Den Maaten and Hilton (2008). The resulting plot can be seen in figure \ref{tsne} and indicates again that a simple distinction between the two groups cannot be made. Yet, there seems to be roughly two groups that differ in heterogeneity: one largely heterogeneous group and one group that is less heterogeneous, though far from homogeneous.
