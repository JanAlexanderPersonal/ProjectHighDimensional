...
```{r}
abbreviation <- c('AUC','IQR', 'LDA','LLE', 'PCA', 'sd')
meaning <-
...
abb_table <- data.frame(abbreviation, meaning)

knitr::kable(abb_table,
             align = 'l',
             col.names = c('abbreviation', 'meaning'))
```
...
The model performance in terms of AUC is extremely similar for the 3 models.
Since LASSO regression is the simplest model, this is preferred.
By chosing cutoff c = 0.23, we can achieve a F1-score of over 0.7. 
...
```
This confustionmatrix clearly shows a sensitivity of `r tab[4] / (tab[3] + tab[4])`, and a specificity of `r tab[1] / (tab[1] + tab[2])`. 
